...
read_when:
  ...
  - Investigating Codex skill root drift after sync-skill-surfaces.sh runs.
---

# Codex Skill Backup Recovery

Codex uses one supported skills root: `~/.agents/skills`. The old
`~/.codex/skills` root is legacy. `agent-tooling/sync-skill-surfaces.sh` migrates
...
backup copy. Run `agent-tooling/sync-skill-surfaces.sh`; it refreshes the active
...
```bash
~/Projects/agent-scripts/agent-tooling/sync-skill-surfaces.sh
...
- `agent-tooling/sync-skill-surfaces.sh` finishes successfully;
