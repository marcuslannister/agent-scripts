...
Foreign source inventories reconcile into `other-skills/<owner>/` staging before selection. `defaultDestinations` plus per-skill overrides decide which staged skills copy to Claude and Codex; an empty override means staged but installed nowhere. Plugin-Claude-only sources use the Claude plugin and a Codex copy when both destinations are supported. Matt keeps separate source entries: Claude plugin delivery plus staged copies selected for Codex.
