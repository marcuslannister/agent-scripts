...
- New `tests/update-repo-skills-test.sh` black-box suite (issue #2): scratch-HOME fixtures assert marked copies for tracked skills, tracked-only enumeration, orphan cleanup on delete/rename, unmarked-dir safety, the drift guard's non-zero exit, idempotence, fail-loud propagation, the no-rsync cp fallback, and empty-tracked-set refusal (a broken enumeration must never mass-delete the Codex surface).
