...
make_fixture() { # fixture upstream
  ...
  mkdir -p "$fixture/scripts" "$fixture/skills" "$fixture/home/.agents/skills" \
    "$fixture/runtime" "$fixture/bin" "$upstream/skills/engineering" "$upstream/skills/deprecated"
...
test -f "$FIXTURE/home/.agents/skills/new-skill/SKILL.md"
test -f "$FIXTURE/skills/new-skill/SKILL.md"
test -f "$FIXTURE/home/.agents/skills/code-review/SKILL.md"
test ! -e "$FIXTURE/skills/code-review"
...
! grep -F '# cli-skills ' "$FIXTURE/.gitignore" >/dev/null
...
grep -F 'add mattpocock/skills --skill * --agent codex --global --yes' "$FIXTURE/npx.log" >/dev/null
grep -F 'remove old-name find-skills --global --agent codex --yes' "$FIXTURE/npx.log" >/dev/null
! grep -F ' update ' "$FIXTURE/npx.log" >/dev/null
...
PARTIAL_UPSTREAM="$TMP_ROOT/partial-upstream"
make_fixture "$PARTIAL" "$PARTIAL_UPSTREAM"
: > "$PARTIAL/npx.log"
set +e
run_topology "$PARTIAL" "$PARTIAL_UPSTREAM" "$PARTIAL/result.json" FAKE_NPX_PARTIAL=1
partial_exit=$?
set -e
test "$partial_exit" -eq 1
jq -e '.status == "failed" and any(.errors[]; contains("Matt skills install failed"))' "$PARTIAL/result.json" >/dev/null
test ! -e "$PARTIAL/skills/new-skill"
test -e "$PARTIAL/skills/old-name"
test -e "$PARTIAL/skills/find-skills"
