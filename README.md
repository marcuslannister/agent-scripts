...
- Waza and claude-mem use manifest-scoped native Claude/Codex adapters. Check mode compares installed versions with each configured marketplace snapshot, reports outdated plugins as planned updates without refreshing caches or managed state, and fails closed when freshness cannot be determined. Unknown installed third-party plugins return decision-required before mutation; Claude official and Codex system plugins are ignored. Claude-mem still requires runnable Bun, uv, and uvx and preserves the shared `~/.claude-mem` worker/database contract.
