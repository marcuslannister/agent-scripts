...
- Claude Code: `~/.claude/skills` only; `scripts/update-skill-topology.sh` reconciles marker-owned per-skill copies selected by `docs/skills-matrix.md`.
...
- Select Claude/Codex destinations by editing `Y`/`N` on `Type: skill` rows in `docs/skills-matrix.md`; generated override blocks in `skill-topology.json` are not hand-edited.
...
- For an external skill-bearing source, add exactly one `skill-topology.json` source and one private adapter registration with its matrix source identity. Stage source-only and npx-only inventories under the matching `other-skills/<owner>/`; never write them directly into `skills/`. Record source defaults and plugin policy in the manifest; never add another public updater.
- Preview with `scripts/update-skill-topology.sh --check`; run `scripts/update-skill-topology.sh` to reconcile and regenerate the matrix report; then run `scripts/verify.sh` before commit.
