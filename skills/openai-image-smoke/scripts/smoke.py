...
def _default_out_dir() -> str:
    ...
    if os.path.isdir(projects_tmp):
        return os.path.join(projects_tmp, f"openai-image-smoke-{_stamp()}")
    return os.path.join(os.getcwd(), "tmp", f"openai-image-smoke-{_stamp()}")
...
def _write_index(out_dir: str, items: list[dict]) -> None:
    html = [
        ...
        "<title>openai-image-smoke</title>",
        "<style>",
