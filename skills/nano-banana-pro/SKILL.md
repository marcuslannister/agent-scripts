...
Run the script using absolute path (do NOT cd to skill directory first). Examples use the Claude Code path `~/.claude/skills/nano-banana-pro`; under Codex the same skill lives at `~/.agents/skills/nano-banana-pro` — substitute accordingly:

**Generate new image:**
```bash
uv run ~/.claude/skills/nano-banana-pro/scripts/generate_image.py --prompt "your image description" --filename "output-name.png" [--resolution 512|1K|2K|4K] [--api-key KEY]
```

**Edit existing image:**
```bash
uv run ~/.claude/skills/nano-banana-pro/scripts/generate_image.py --prompt "editing instructions" --filename "output-name.png" --input-image "path/to/input.png" [--resolution 512|1K|2K|4K] [--api-key KEY]
```
