...
- For third-party/open-source repos not owned by `steipete`, prefer cloning under `~/Projects/oss`.
- Tailscale SSH: when Peter says “MacBook” or “Mac Studio”, run commands on that machine via SSH (e.g. `ssh steipete@peters-mac-studio-1 '...'`); find device names/IPs via `tailscale status`.
...
- This repo no longer uses a `./runner` wrapper; run commands directly. Use `bin/git` + `bin/rm` for the guardrails that used to live in runner.
...
- Prefer Codex background execution for long-running jobs; use tmux only when you need an interactive/persistent session (debuggers, servers).
...
- After the user explicitly approves a guarded/destructive git operation, re-run with `AGENT_GIT_CONSENT=1` (works with `bin/git`).
...
### bird
- Twitter/X CLI at `~/Projects/bird/bird`. Post tweets, reply, read threads, search.
- Key commands: `bird tweet "<text>"`, `bird reply <tweet-id-or-url> "<text>"`, `bird read <url>`, `bird replies <url>`, `bird thread <url>`, `bird search "<query>"`, `bird mentions`, `bird whoami`.
- Uses Firefox cookies by default; pass `--firefox-profile` to switch profiles.

### sonoscli
- Sonos speaker control CLI at `~/Projects/sonoscli/bin/sonos`. Built in Go.
- Key commands: `sonos discover`, `sonos status --name "Room"`, `sonos play/pause/stop --name "Room"`, `sonos volume set --name "Room" 25`, `sonos group status`, `sonos group join --name "A" --to "B"`.
- **Known issue**: SSDP multicast may fail on some networks; use `--ip <speaker-ip>` as fallback. Default HTTP client can be slow due to keep-alives; a fix (DisableKeepAlives) is pending.
- Supports Spotify via SMAPI: `sonos smapi search --service "Spotify" --category tracks "query"`.

### peekaboo
- Screenshot and screen inspection CLI at `~/Projects/Peekaboo`. Capture screens, windows, apps; OCR; click automation.
- Key commands: `peekaboo capture`, `peekaboo see`, `peekaboo click`, `peekaboo list`, `peekaboo tools`, `peekaboo permissions status`.
- Requires Screen Recording + Accessibility permissions.
- See `~/Projects/Peekaboo/docs/commands/` for detailed command docs.

### sweetistics
- Twitter/X analytics app at `~/Projects/sweetistics`. Tauri desktop app.
- Use for deeper Twitter data analysis beyond what `bird` provides.

### bin/git / scripts/git-policy.ts
- Git shim that enforces guardrails (blocks `git add`/`git commit`, gates guarded/destructive subcommands). Use `bin/git ...`.
- For guarded/destructive git operations: get explicit user approval, then re-run with `AGENT_GIT_CONSENT=1`.
...
- Trash-backed `rm` replacement; use `bin/rm ...` instead of `rm` so deletes are recoverable.
...
- Chrome DevTools helper without running an MCP server. Key commands: `start` (launch Chrome with remote debugging), `nav <url>` (navigate), `eval <js>` (run JS in the active tab), `screenshot` (viewport PNG to /tmp), `pick <msg>` (interactive element picker), `cookies`, `inspect`, `kill`. Launch via `bin/browser-tools --help`. Edit `scripts/browser-tools.ts`, rebuild with `bun build scripts/browser-tools.ts --compile --target bun --outfile bin/browser-tools`.
